...
const CLERK_JWKS_URL =
  ...
  process.env.CLERK_FRONTEND_API_URL
    ? `${process.env.CLERK_FRONTEND_API_URL}/.well-known/jwks.json`
    : "";
...
async function verifyClerkSessionToken(sessionToken) {
  ...
  try {
    ...
    if (!response.ok || !data) {
      debugLog("Clerk verify failed:", response.status, data);
      return null;
    }
...
async function fetchClerkUser(userId) {
  ...
  try {
    ...
    if (!response.ok || !data) {
      debugLog("Clerk user fetch failed:", response.status, data);
      return null;
    }
...
export default async function handler(req, res) {
  ...
  const cacheKey = `search:v29:${normalizedQuery}`;
  ...
  const semanticKey = semanticFingerprint
    ? `semantic:v15:${semanticFingerprint}`
    : "";
